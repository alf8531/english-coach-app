...
import { ProficiencyLevel, TextSample, AnalysisResult, VocabItem, ChatMessage, SRSLevel, HighlightedPhrase } from '../types';
...
const App: React.FC = () => {
  const [view, setView] = useState<'library' | 'practice' | 'analysis' | 'quiz' | 'flashcards' | 'chat' | 'listening' | 'writing' | 'deepchat'>('library');
  ...
  const handleSaveToDeckFromTooltip = (phrase: HighlightedPhrase, sentence: string) => {
    const newItem: VocabItem = {
      ...
      word: phrase.phrase,
      phonetic_spelling: phrase.phoneticSpelling,
      clear_definition: phrase.definition,
      examples_array: sentence ? [sentence, ...phrase.examples] : phrase.examples,
      srsLevel: 'New',
      dateAdded: Date.now()
    };
  ...
  const updateSRS = (id: string, level: SRSLevel) => {
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row" onClick={() => setActiveTooltip(null)}>
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 w-64 bg-slate-900 text-white transform transition-transform duration-300 ease-in-out md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:flex md:flex-col`}>
        ...
        <nav className="flex-1 px-4 space-y-2 mt-4 md:mt-0 overflow-y-auto">
          <button onClick={() => { setView('library'); setIsSidebarOpen(false); }} className={`w-full text-left flex items-center gap-4 px-5 py-4 rounded-2xl transition-all ${view === 'library' || view === 'practice' || view === 'analysis' ? 'bg-indigo-600 shadow-lg' : 'hover:bg-slate-800 text-slate-400 font-bold'}`}>Reading Lab</button>
          <button onClick={() => { setView('deepchat'); setIsSidebarOpen(false); }} className={`w-full text-left flex items-center gap-4 px-5 py-4 rounded-2xl transition-all ${view === 'deepchat' ? 'bg-indigo-600 shadow-lg' : 'hover:bg-slate-800 text-slate-400 font-bold'}`}>Deep Chat <span className="text-[10px] bg-amber-500 text-slate-900 px-1.5 py-0.5 rounded font-black uppercase">New</span></button>
          <button onClick={() => { setView('listening'); setIsSidebarOpen(false); }} className={`w-full text-left flex items-center gap-4 px-5 py-4 rounded-2xl transition-all ${view === 'listening' ? 'bg-indigo-600 shadow-lg' : 'hover:bg-slate-800 text-slate-400 font-bold'}`}>Scene Simulator</button>
          <button onClick={() => { setView('writing'); setIsSidebarOpen(false); }} className={`w-full text-left flex items-center gap-4 px-5 py-4 rounded-2xl transition-all ${view === 'writing' ? 'bg-indigo-600 shadow-lg' : 'hover:bg-slate-800 text-slate-400 font-bold'}`}>Writing Workshop</button>
          <button onClick={() => { setView('flashcards'); setIsSidebarOpen(false); }} className={`w-full text-left flex items-center gap-4 px-5 py-4 rounded-2xl transition-all ${view === 'flashcards' ? 'bg-indigo-600 shadow-lg' : 'hover:bg-slate-800 text-slate-400 font-bold'}`}>Vocab Bank ({deck.length})</button>
          <button onClick={() => { setView('chat'); setIsSidebarOpen(false); }} className={`w-full text-left flex items-center gap-4 px-5 py-4 rounded-2xl transition-all ${view === 'chat' ? 'bg-indigo-600 shadow-lg' : 'hover:bg-slate-800 text-slate-400 font-bold'}`}>Deep Think Text</button>
        </nav>
        <div className="p-6 border-t border-slate-800 mt-auto">
          <button onClick={() => { setShowLiveCoach(true); setIsSidebarOpen(false); }} className="w-full py-4 bg-amber-500 rounded-2xl text-slate-900 font-black hover:bg-amber-400 shadow-lg active:scale-95 transition-all">Live Voice Coach</button>
        </div>
      </aside>
      ...
      <main ref={mainContainerRef} className="flex-1 p-4 md:p-12 min-h-screen relative overflow-x-hidden">
        <SelectionMenu
          ...
          onAddToDeck={(t) => handleSaveToDeckFromTooltip({ phrase: t, phoneticSpelling: '...', definition: 'User selected', examples: [] }, '')}
        ...
        {view === 'analysis' && currentSample && <AnalysisView result={analysisResult} isLoading={isAnalyzing} targetSample={currentSample} userAudioUrl={userAudioUrl} onRetry={() => { setView('practice'); setUserAudioUrl(null); }} onNew={() => { if (generationMode === 'ai') { handleGenerate(); } else { setView('library'); } }} onHearCoach={() => setShowLiveCoach(true)} onAskCoach={() => setVoiceAskData({ text: currentSample.title })} onSaveTerm={t => handleSaveToDeckFromTooltip(t, '')} />}
        {view === 'listening' && <ListeningLab />}
        {view === 'writing' && <WritingWorkshop />}
        {view === 'flashcards' && <FlashcardsView deck={deck} onUpdateSRS={updateSRS} />}
        {view === 'deepchat' && <DeepChat onBack={() => setView('library')} />}
