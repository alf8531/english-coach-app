...
const DeepChat: React.FC<DeepChatProps> = ({ onBack }) => {
  ...
  // ================= SHARED RENDERERS =================
  const renderRoleplayAnalysisCard = (analysis: PronunciationAnalysis) => {
    return (
            ...
            <>
                ...
                <div className="p-8 bg-white border-t border-slate-100 flex justify-center pb-10">
                     <button 
                        ...
                        disabled={isLoading}
                        className={`w-full max-w-sm py-6 rounded-[2.5rem] font-black text-xl transition-all shadow-2xl flex items-center justify-center gap-3 select-none active:scale-95 ${
                            isRecording ? 'bg-red-500 text-white scale-105 ring-8 ring-red-100' : 'bg-slate-900 text-white hover:bg-indigo-600'
                        }`}
                        >
